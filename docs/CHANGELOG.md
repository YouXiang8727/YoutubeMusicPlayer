...
### Added
- **探索頁「為你推薦」區塊**（2026-09，置於「台灣熱門音樂」rail 之上、僅 rail 模式渲染）：以**最近加入播放清單**的歌曲為種子（跨全部播放清單、依 addedAt 最新在前、videoId 去重），抓取各種子 watch page **Related Songs**（NewPipe `getRelatedItems()`，extractor v0.26.5 確認可用）後**跨種子共現評分**排序。資料鏈：`core:domain` 新增 `RecommendationRepository`（`recommendationsFor(seeds, limit)`——種子為空回空不觸網、實作端排除種子自身與「已在播放清單」的歌曲、錯誤一律 `Result.failure` 封裝）、`FetchRecommendationsUseCase`（`SEED_LIMIT=5`／`RECOMMENDATION_LIMIT=10`，limit clamp 至 `1..RECOMMENDATION_LIMIT`，種子原樣透傳）、`ObserveRecentPlaylistItemsUseCase`（薄轉發 `PlaylistRepository.observeRecentItems`，種子來源）；`PlaylistRepository`／`PlaylistDao` 新增 `observeRecentItems(limit)`（跨全部清單 addedAt 降序、videoId 去重保留最新、**不開新表不 bump DB version**）。`core:data` 新增 `RelatedStreamsDataSource`（NewPipe watch page，走 `@StreamProfile` 乾淨 client → `OkHttpDownloader` 與串流解析同身份；NewPipe 初始化與 `NewPipeStreamSource` 共用 `remote.stream.ensureExtractorInitialized`（`NewPipeInitializer`，全程序唯一延遲 init＋double-checked lock）；`StreamInfoItem` 過濾＋videoId 萃取＋秒數格式化拆 internal 純函數供 JVM 測試）與 `RecommendationRepositoryImpl`（各 seed **併發**抓取、單一 seed 失敗視同空清單、全部失敗回 `Result.failure`；`rankRecommendations` 純函數＝候選出現於幾個不同 seed 的 related 集合→共現分數降序→同分 title 字元序→排除種子＋已知（`observeRecentItems(1000)` 快照）→取 limit）；Hilt `RepositoryModule` 補 `RecommendationRepository` @Binds。UI：`feature:discover` `DiscoverViewModel` 新增 `RecommendationState`（`seedEmpty`/`loading`/`items`/`error`）並於 init 觀察種子、`DiscoverIntent.RecommendationRefresh`（換一批＝以最近種子重新抓取，種子為空 no-op）；`DiscoverScreen` 新增 `RecommendationSection` 置於熱門榜單上方——種子為空顯示「加入歌曲開始推薦」、成功顯示「為你推薦」rail＋「換一批」、失敗顯示錯誤＋重試。新增測試：`RecommendationRepositoryImplTest`（跨種子共現／排除已收藏／全部失敗／空種子）、`RelatedStreamsDataSourceTest`（映射＋videoId 萃取＋時長格式化）、`FetchRecommendationsUseCaseTest`（limit clamp）、`ObserveRecentPlaylistItemsUseCaseTest`、`PlaylistRepositoryImplTest` 擴充（observeRecentItems 4 案例）、`DiscoverViewModelTest` 推薦 6 案例與 `DiscoverScreen` 2 組 Preview（Recommendation Rail 深/淺、Empty 深/淺）
- **探索頁載入改為 region-level skeleton**：新增 `core:ui` 共用元件 `VideoRailSkeleton`（影片橫向 rail 載入骨架——標題列＋橫向卡片 placeholder，卡片輪廓與 `ChartRailItem`/`ChartRail` 對齊（寬 140dp、縮圖高 80dp、LazyRow 間距 12dp），整層 pulse 透明度動畫由 Compose `animation-core` 提供、**未新增任何依賴**）；`feature:discover` `DiscoverScreen` 的 `TrendingSection` loading 分支由「4 個大間距 `CircularProgressIndicator`」改為呼叫 `VideoRailSkeleton`（loading 僅發生於 rail 模式，完整榜單展開無 loading 狀態，不需 full-chart 變體）。刪除反映舊畫面的 `DiscoverScreenLoadingPreview`，新增 `DiscoverScreenSkeletonPreview`（深/淺色，四區 skeleton 並排驗證）；`VideoRailSkeleton` 附 core-ui 深/淺色 Preview。命名採通用語意（非 Trending 專用），未來搜尋結果等橫向 rail 載入處可複用
- **播放模式（隨機／循環）持久化**：切換隨機／循環後自動保存、下次播放（MusicService 建立）自動還原上次模式。`core:domain` 新增 `PlaybackPreferences`（`shuffleEnabled`＋`repeatMode`，`RepeatMode` 僅 ALL/ONE）與 `PlaybackPreferencesRepository`（`get()`/`save()`）；`core:data` 新增 `PlaybackPreferencesRepositoryImpl`（DataStore Preferences 1.2.1，delegate `playback_preferences`；keys：`shuffle_enabled` boolean／`repeat_mode` string 存 `RepeatMode.name`；缺 key 或 unknown 字串 fallback 預設 shuffle=false／repeatMode=ALL；DataStore 內部自管 IO、直接呼叫），Hilt `RepositoryModule` 補 `PlaybackPreferencesRepository` @Binds。`MusicService` onCreate 以持久化值還原（取代硬編碼 `Player.REPEAT_MODE_ALL`，無檔回傳預設行為一致），**還原防覆蓋 user 早期切換（flag 防護）**：`playbackModeUserModified` flag 於 listener 切換時設 true，還原 read 前後雙重檢查（DataStore 首次讀取數百 ms 期間 user 可能已切換模式）避免用舊值覆蓋。新增 `PlaybackPreferencesRepositoryImplTest`（4 案例：空 store 預設／save shuffle=true repeatMode=ALL 回讀／save shuffle=true repeatMode=ONE 回讀／unknown repeat_mode fallback ALL）
