...
- `feature.player.service.MusicService`：Media3 `MediaSessionService`
  ...
  - 通知由系統自動產生：歌名、進度條（可拖曳 seek）、播放/暫停；上一首/下一首改為 **custom layout 常駐按鈕**（`SLOT_BACK`/`SLOT_FORWARD` 取代系統按鈕，避免單曲循環播到佇列最後一首時「下一首」icon 消失；以自訂 session command 實作，佇列無可去曲目時於 service 端重播目前曲目開頭）；隨機與循環亦為 custom layout 按鈕（custom command，使用 Media3 官方 `CommandButton.ICON_*`：`ICON_PREVIOUS/NEXT`、`ICON_SHUFFLE_ON/OFF`、`ICON_REPEAT_ALL/ONE`，`ICON_SHUFFLE_OFF` 為官方 disabled 色）；通知本體點擊經 `MediaSession.setSessionActivity`（contentIntent）將 App 帶回前景
