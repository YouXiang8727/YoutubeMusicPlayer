...
### core:data
- `local.PlaylistEntity` / `PlaylistItemEntity`：Room Entity（持久化細節，不外洩）；與 Domain Model 互轉的 mapper 在同檔（`PlaylistItemEntity` 以**複合主鍵 `(playlistId, videoId)`**（DB v7 起）——同一影片可同時存在於多個歌單，`insertItem(REPLACE)` 以「歌單＋影片」為衝突單位；修前單一 videoId 全域 PK 會使 KeepBoth 匯入跨歌單覆蓋舊歌單 items；含 `duration` 可空 TEXT 欄位與 `streamFailedAt` 可空 INTEGER 欄位；`PlaylistEntity.name` 具**唯一索引**（DB v6 起）——資料層最終防線，insert 衝突（IGNORE）回傳 -1 由 Repository 轉為語意化例外；DB version 7，`MIGRATION_2_3` = `ALTER TABLE playlist_items ADD COLUMN duration TEXT`、`MIGRATION_4_5` = `ALTER TABLE playlist_items ADD COLUMN streamFailedAt INTEGER`、`MIGRATION_5_6` = 建 `playlists.name` 唯一索引、`MIGRATION_6_7` = 重建 `playlist_items` 為複合主鍵）
...
- `feature:playlist` 歌單列表頁 JSON **匯出/匯入**（2026-09）：`PlaylistListViewModel` 注入 `ExportPlaylistUseCase`（`playlistId → String?`）／`ExportAllPlaylistsUseCase`（`→ String?`，v2 bundle）／`ImportPlaylistUseCase`（`(json, onConflict) → PlaylistImportResult?`，v1/v2 相容、**衝突詢問制**），新增 `PlaylistListIntent.Export(id)`／`ExportAll`／`Import(json)` 與 `exportResult: SharedFlow<String>`（Export / ExportAll 皆 emit JSON）。**匯出（單一）**：每張歌單卡片 overflow menu（`MoreVert` → `DropdownMenu`：匯出 JSON／重新命名／刪除，取代原 Edit/Delete 圖示）→ VM Export → Route 收集 `exportResult` 後以 **MediaStore 直寫**固定資料夾 `Download/MyMediaPlayer/`（`MediaStore.Downloads.EXTERNAL_CONTENT_URI`＋`RELATIVE_PATH`，minSdk 33 > API 29 全裝置支援、不需 fallback；檔名 `MyMediaPlayer_<歌單名>.json`，非法字元以 `_` 取代、空白兜底 playlist；**檔名衝突由 MediaProvider 自動加「 (N)」後綴**，不需手動處理）。**匯出（全部）**：頁首「匯出全部」`TextButton` → `ExportAll` → 同一 `exportResult` 流程，檔名 `MyMediaPlayer_Backup_<yyyyMMdd>.json`。**匯入（衝突詢問制，2026-09 改 App 內選檔取代 GetContent）**：頁首「匯入歌單」`TextButton` → 開啟 `ImportBackupSheet`（ModalBottomSheet，標題「匯入歌單」＋右上「重新掃描」按鈕）；**開啟時才掃描**——以 `PlaylistImportHelper.queryPlaylistBackups()`（MediaStore `MediaStore.Downloads.EXTERNAL_CONTENT_URI`＋`RELATIVE_PATH = Download/MyMediaPlayer/`＋`DISPLAY_NAME LIKE '%.json'`，依 `DATE_MODIFIED DESC` 排序；失敗/查無回空清單）列 `Download/MyMediaPlayer/` 下 `.json` 備份檔（顯示名＋修改日期 `yyyy-MM-dd HH:mm`＋大小 B/KB/MB 格式化），空狀態提示「Download/MyMediaPlayer/ 沒有備份檔，請先執行匯出」；**每筆備份檔右側附「匯入」／「刪除」兩顆按鈕**（左側檔名＋日期＋大小區維持可點＝匯入，右側 `TextButton`「匯入」與 `Icons.Filled.Delete` IconButton「刪除備份」獨立不冒泡）——**刪除需確認**：`PlaylistListRoute` 以 `pendingDelete` state 驅動 AlertDialog（標題「刪除備份檔？」、內容「確定要刪除「<displayName>」嗎？此操作無法復原。」、取消／刪除〔error 色〕，sheet 維持開啟），確認後 `withContext(Dispatchers.IO)` 呼叫 `deletePlaylistBackup(uri)`（`contentResolver.delete`）——成功自動重掃清單＋Snackbar「已刪除 <displayName>」、失敗 Snackbar「刪除備份失敗」（MediaStore delete 屬 ContentResolver 真機行為，不加單測）；**點擊某檔才讀取內容**——`readPlaylistBackup(uri)` 讀取全文（失敗以 Snackbar「讀取備份失敗」提示）→ `Import(json)`；遇同名歌單時 VM 以 `importConflict: StateFlow<ImportConflictInfo?>` 對外暴露並以 `CompletableDeferred` 暫停等待決策，`PlaylistListScreen` Scaffold 最上層顯示 `ImportConflictDialog`（取代／兩者皆保留／取消＋可選「全部套用」勾選框，`onImportConflictDecision(decision, applyToAll)`：勾選時快取決策、後續衝突直接沿用不再彈窗；每次 Import 開始重設）；完成後以結果統計 Snackbar 回饋（「已匯入：新增 X、取代 Y、保留兩者 Z」〔只列非零項〕／「匯入已中斷：…」／「已取消匯入」／「匯入失敗：JSON 格式無法辨識」／「沒有任何歌單可匯入」；Export 找不到歌單「找不到該歌單」／無歌單可匯出「尚無任何歌單可匯出」）。檔案 I/O（MediaStore）只存在 Route（Compose 層）＋同一 package 的 `PlaylistExportHelper.kt`（`writePlaylistBackup`：`IS_PENDING=1` 插入 → 寫入 json bytes → 更新為 0，**任一失敗刪除該 uri 不留半成品**；`queryStoredDisplayName`：碰撞後查回實際檔名）與 `PlaylistImportHelper.kt`（`queryPlaylistBackups`：依 `RELATIVE_PATH`＋`.json` 過濾掃描回 `List<PlaylistBackupFile>`，失敗回空；`readPlaylistBackup`：讀 Uri 全文，失敗回 null；`deletePlaylistBackup`：`contentResolver.delete(uri)` 刪除單一備份，回傳 >0 視為成功、exception/0 回 false 不 throw；檔名/時間/大小格式化拆 UI 層純函式 `formatBackupModifiedDate`／`formatBackupFileSize` 供 JVM 測試），寫入/讀取包 `withContext(Dispatchers.IO)`，ViewModel 不碰 Context/檔案；匯出寫入**成功/失敗皆以 Snackbar 回饋**（成功「已儲存 <實際檔名>（Download/MyMediaPlayer/）」；失敗「匯出失敗，請再試一次」）。**新建／重新命名重名防呆（2026-09，承接資料鏈唯一約束）**：`Create`／`Rename` handler 包 `runCatching`——遇 `PlaylistNameConflictException` 以 Snackbar 直接提示「已存在同名歌單「X」」，其餘例外維持「建立失敗：…」／「重新命名失敗：…」，成功訊息（「已建立「X」」／「已重新命名為「X」」）不變；`CreatePlaylistDialog` 以 `state.playlists` 轉 `existingNames` 即時阻擋重名，`RenamePlaylistDialog` 以「其他歌單」清單（排除目前 id，故改名為自己目前名稱仍合法可確認）同步防呆
...
| 歌單匯出/匯入固定 `Download/MyMediaPlayer/` 資料夾（MediaStore `RELATIVE_PATH`，2026-09 起取代 SAF） | 使用者無法自選存檔位置；**其他位置的備份（含系統檔案選擇器可達的任意目錄）無法匯入**；Download 資料夾可能被使用者手動刪除／清空 | 匯出/匯入同資料夾彼此對應（匯出→匯入閉環）；minSdk 33 > API 29 全裝置支援 `RELATIVE_PATH` 子資料夾，不需 fallback；檔名衝突由 MediaProvider 自動加「 (N)」後綴、成功 Snackbar 查回實際檔名＋顯示完整路徑，失敗明確提示；匯入以 App 內掃描（含重新掃描）取代系統檔案選擇器，空狀態提示先執行匯出；**備份檔可於匯入清單直接刪除**（每筆「刪除」按鈕＋確認 AlertDialog，成功自動重掃清單，App 內即可清理佔空間的舊備份，不需手動進檔案管理員）；行為可預期，屬使用者可接受取捨 | C |
