...
- `feature:playlist` 歌單列表頁 JSON **匯出/匯入**（2026-09）：`PlaylistListViewModel` 注入 `ExportPlaylistUseCase`（`playlistId → String?`）／`ExportAllPlaylistsUseCase`（`→ String?`，v2 bundle）／`ImportPlaylistUseCase`（`json → Long?`，v1/v2 相容），新增 `PlaylistListIntent.Export(id)`／`ExportAll`／`Import(json)` 與 `exportResult: SharedFlow<String>`（Export / ExportAll 皆 emit JSON）。**匯出（單一）**：每張歌單卡片 overflow menu（`MoreVert` → `DropdownMenu`：匯出 JSON／重新命名／刪除，取代原 Edit/Delete 圖示）→ VM Export → Route 收集 `exportResult` 後以 `CreateDocument("application/json")` SAF 存檔（檔名 `MyMediaPlayer_<歌單名>.json`，非法字元以 `_` 取代、空白兜底 playlist）。**匯出（全部）**：頁首「匯出全部」`TextButton` → `ExportAll` → 同一 `exportResult` 流程，檔名 `MyMediaPlayer_Backup_<yyyyMMdd>.json`。**匯入**：頁首「匯入歌單」`TextButton` → `GetContent()`（mime `application/json`）→ 讀取 Uri 全文（`openInputStream().bufferedReader().readText()`）→ `Import(json)`；成功/失敗以 Snackbar 提示（「已匯入歌單」／「匯入失敗：JSON 格式無法辨識」／Export 找不到歌單「找不到該歌單」／無歌單可匯出「尚無任何歌單可匯出」）。檔案 I/O（SAF）只存在 Route（Compose 層），ViewModel 不碰 Context/檔案；匯出寫入**成功/失敗皆以 Snackbar 回饋**（成功顯示檔名＋位置，位置取不到時僅檔名；失敗「匯出失敗，請再試一次」）
